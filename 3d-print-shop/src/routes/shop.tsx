...
const searchSchema = z.object({
  category: fallback(z.enum(["All", "Decor", "Lighting", "Tableware", "Games", "Office"]), "All").default("All"),
...
export const Route = createFileRoute("/shop")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse the Forma collection of sculptural 3D printed objects.",
      },
      ...
      <div className="flex items-center justify-between gap-4 mb-8 md:mb-12 sticky top-16 z-20 bg-background/85 backdrop-blur-md py-3 -mx-5 px-5 md:mx-0 md:px-0 md:static md:bg-transparent md:backdrop-blur-none border-b border-border md:border-0">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            // Use history API replace via Link not possible here; use anchor change via window.location alt:
            const url = new URL(window.location.href);
            url.searchParams.set("sort", v);
            window.history.replaceState(null, "", url.toString());
            // Force a refresh by dispatching popstate
