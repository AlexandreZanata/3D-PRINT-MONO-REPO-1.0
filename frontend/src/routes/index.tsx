...
import { HeroSkeleton } from "@/organisms/HeroSkeleton/HeroSkeleton";
...
function Home() {
  const { data: productList, isLoading: productsLoading } = useProducts({ limit: 6, isActive: true });
  ...
  return (
    <div>
      {/* Hero — skeleton while settings load */}
      {settingsLoading ? (
        <HeroSkeleton />
      ) : (
      ...
      {/* Featured — skeleton grid while products load */}
